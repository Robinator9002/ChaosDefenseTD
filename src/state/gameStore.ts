...
/**
 * Extends the base GameState with instances of our logic managers.
 */
...
export const useGameStore = create<GameStateWithManagers & GameActions>((set, get) => ({
    ...
    damageEnemy: (enemyId, amount) =>
        set((state) => {
            ...
            // Check for vulnerability effects and apply damage multiplier
            const vulnerabilityEffect = enemy.effects.find(
                (e) => e.id === 'vulnerability' || e.id === 'elemental_vulnerability',
            );
            if (vulnerabilityEffect) {
                finalDamage *= vulnerabilityEffect.potency;
            }
    ...
    placeTower: (config, tile) => {
        const newTower: TowerInstance = {
            ...
            currentPierce: config.attack?.data.pierce ?? 0,
            currentChains: config.attack?.data.chains ?? 0,
        };
        set((state) => ({
    ...
    spawnProjectile: (tower, target) => {
        ...
        const newProjectile: ProjectileInstance = {
            ...
            onBlastEffects: attackData.on_blast_effects,
            hitEnemyIds: [],
        };
        set((state) => ({
    ...
    update: (dt) => {
        ...
        // The order of updates is critical for correct behavior.
